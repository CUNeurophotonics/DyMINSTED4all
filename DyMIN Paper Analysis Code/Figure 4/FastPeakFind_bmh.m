function  [cent, varargout]=FastPeakFind_bmh(d, sigma, thres, filt ,edg, res, fid)
...
if (nargin < 3)
    thres = .8*(max([min(max(d,[],1))  min(max(d,[],2))])) ;
%     thres = 40; %this is modified by BMH might need to implement a better way to do this
end

if (nargin < 4)
%     filt = (fspecial('gaussian', hsize, sigma)); %if needed modify the filter according to the expected peaks sizes
%     filt = imgaussfilt(d, sigma);
end
...
%         d=conv2(single(d),filt,'same') ;
        ...
        %% it seems like this only works with variable types single or double
